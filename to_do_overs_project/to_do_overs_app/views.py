...
from .cipher_functions import encrypt_text
from .forms import TasksModelForm
from .models import Tags, Tasks, Users
from .to_do_overs_data import ToDoOversData
...
def index(request):
    ...
    """
    return render(request, "to_do_overs_app/index.html")
...
def login(request):
    ...
    if session_class.login(password):
        ...
        return redirect("to_do_overs_app:dashboard")
    else:
        messages.warning(request, "Login failed.")
        return redirect("to_do_overs_app:index")
...
def dashboard(request):
    ...
    if session_class.logged_in:
        ...
        return render(
            request,
            "to_do_overs_app/dashboard.html",
            {
                "username": username,
                "tasks": task_list,
            },
...
def create_task(request):
    ...
    if session_class.logged_in:
        ...
        return render(request, "to_do_overs_app/create_task.html", {"form": form})
...
def create_task_action(request):
    ...
    Returns:
        Redirects to index if user is logged out. Otherwise attempts to create task.
        If creation is successful, redirects to dashboard.
        If creation fails, redirect back to create task page.
    """
    session_class = jsonpickle.decode(request.session["session_data"])
    if session_class.logged_in:
        ...
        if form.is_valid():
            ...
            if int(session_class.task_days) < 0:
                ...
                return redirect("to_do_overs_app:create_task")
...
def delete_task(request, task_pk):
    ...
    if logged_in_user.pk == owner.pk:
        return render(request, "to_do_overs_app/delete_task.html", {"task": task})
    else:
...
def edit_task(request, task_pk):
    ...
    if logged_in_user.pk == owner.pk:
        ...
        return render(request, "to_do_overs_app/edit_task.html", {"form": form, "task_pk": task_pk})
    else:
...
def edit_task_action(request, task_pk):
    ...
    if logged_in_user.pk == owner.pk:
        ...
        if form.is_valid():
            ...
            if int(session_class.task_days) < 0:
                messages.warning(request, "Invalid repeat day number.")
                return redirect("to_do_overs_app:edit_task", task_pk)
